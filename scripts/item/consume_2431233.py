...
rewardID = int(random.choice(list(herbNameDict.keys())))


if sm.getEmptyInventorySlots(4) >= 1:
    sm.consumeItem(2431233)
    sm.giveItem(rewardID, herbAmount)
    chr.chatMessage(ChatType.GameDesc,"You have gained items in the Etc tab (" + herbNameDict.get(str(rewardID)) + " " + str(herbAmount) + ")")
